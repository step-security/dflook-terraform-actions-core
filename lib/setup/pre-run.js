 ...
 * text.
 */
export async function runPreRunCommands(script, options = {}) {
    if (!script?.trim())
        return false;
    ...
    try {
        const exitCode = await withWorkflowCommandsStopped(() => exec(options.shell ?? 'bash', ['-xeo', 'pipefail', path], { ignoreReturnCode: true }));
        if (exitCode !== 0) {
...
//# sourceMappingURL=pre-run.js.map