 ...
 * itself.
 */
export function digestFor(sums, archiveName) {
 ...
 */
export function assertDigest(path, archiveName, expected) {
    const actual = sha256OfFile(path);
    if (actual === expected)
        return;
    throw new VerificationError(`Checksum mismatch for ${archiveName}: expected ${expected}, got ${actual}. ` +
