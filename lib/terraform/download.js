...
import { HASHICORP_KEY_SUFFIX, HASHICORP_SIGNING_KEY, VerificationError, assertDigest, digestFor, } from './verify.js';
...
async function fetchTo(url, destination) {
    ...
    if (!response.ok) {
        throw new DownloadError(response.status === 404
 ...
 */
async function verifySumsSignature(sumsPath, signaturePath) {
    const exitCode = await exec('gpg', ['--assert-signer', HASHICORP_SIGNING_KEY, '--verify', signaturePath, sumsPath], { ignoreReturnCode: true, silent: true });
    if (exitCode !== 0) {
        throw new VerificationError('Could not verify the signature on the checksums file. The download was not used.');
    }
}
/**
 ...
 */
export async function acquireTerraform(version, options = {}) {
 ...
 */
export async function acquireOpenTofu(version) {
    const platform = releasePlatform();
    const arch = releaseArch();
    const text = version.toString();
...
export async function acquire(version, options = {}) {
