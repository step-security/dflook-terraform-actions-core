...
export function releasePlatform(): string {
  ...
  if (!platform) throw new UnsupportedPlatform(`No release is published for '${process.platform}'`)
  return platform
}
...
export function releaseArch(): string {
  ...
  if (!arch) throw new UnsupportedPlatform(`No release is published for '${process.arch}'`)
  return arch
}
