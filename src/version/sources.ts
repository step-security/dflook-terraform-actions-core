 ...
 */
export function fromRequiredVersion(
 ...
 * exist.
 */
export function parseTfenv(contents: string, versions: Version[]): Version | undefined {
...
export function fromTfswitch(modulePath: string): Version | undefined {
...
export function parseToolVersions(contents: string, versions: Version[]): Version | undefined {
  for (const line of contents.split('\n')) {
    const match = /^\s*terraform\s+([^\s#]+)/.exec(line.trim())
    if (!match) continue

    if (match[1] === 'latest') return latestFinalVersion(versions)
...
export function fromEnvironment(
  ...
  versions: Version[]
): { version?: Version; variable?: string; unmatchedConstraint?: string } {
  const variable =
    env.TERRAFORM_VERSION !== undefined
      ? 'TERRAFORM_VERSION'
  ...
  if (!constraints.length) return { variable }

  const allowed = applyConstraints(versions, constraints)
  if (!allowed.length) return { variable, unmatchedConstraint: expression }
 ...
 * apply used.
 */
export function fromLocalState(modulePath: string, openTofu = false): Version | undefined {
  const contents = tryRead(join(modulePath, 'terraform.tfstate'))
  if (contents === undefined) return undefined

  try {
    const state = JSON.parse(contents) as { serial?: number; terraform_version?: string }
    if (!state.serial || state.serial <= 0) return undefined
    if (!state.terraform_version) return undefined
...
export function sourceExists(modulePath: string, filename: string): boolean {
